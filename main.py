...
#
# This is a pure python implementation of the Triple DES encryption algorithm.
# It is pure python to avoid cross-platform issues, for optimization and learning perspectives
#
# Requirements:
...
# Encryption Phase-I
# Plaintext x Key 1 | E(K1,P)
# Encryption with K1
encrypt(left, right, _roundkey_arr_K1)

phasei_ciphertext = left + right
print(f'{bcolors.OKGREEN}Phase-I Ciphered Text (by Key-1) in Bits is: {phasei_ciphertext}')
phasei_st_cipher = ''
phasei_binary = []
for phasei_bytes in range(8):
    phasei_binary.append(int(phasei_ciphertext[phasei_bytes], 2))
    phasei_st_cipher += chr(phasei_binary[phasei_bytes])

print(f'Phase-I Ciphered Text (by Key-1) in ASCII Characters is:{phasei_st_cipher}')
print(f'{bcolors.OKCYAN}Left at the end of Encryption Phase-I is:{left}')
print(f'Right at the end of Encryption Phase-I is:{right}')

# Encryption Phase-II
# Previous Result x Key 2 | D(K2,E(K1,P))
# Decryption with K2
decrypt(left, right, _roundkey_arr_K2)  # calling decrypt method for cipher text!!!

phaseii_ciphertext = left + right
print(f'{bcolors.OKGREEN}Phase-II Ciphered Text (by Key-2) in Bits is: {phaseii_ciphertext}')
phaseii_st_cipher = ''
phaseii_binary = []
for phaseii_bytes in range(8):
    phaseii_binary.append(int(phaseii_ciphertext[phaseii_bytes], 2))
    phaseii_st_cipher += chr(phaseii_binary[phaseii_bytes])

print(f'Phase-II Ciphered Text (by Key-2) in ASCII Characters is:{phaseii_st_cipher}')
print(f'{bcolors.OKCYAN}Left at the end of Encryption Phase-II is:{left}')
print(f'Right at the end of Encryption Phase-II is:{right}')

# Encryption Phase-III (Final Phase)
# Previous Result x Key 1 | E(K1,D(K2,E(K1,P)))
# Encryption with K1
encrypt(left, right, _roundkey_arr_K1)

phaseiii_ciphertext = left + right
print(f'{bcolors.OKGREEN}Required Ciphered Text (by Key-1) in Bits is: {phaseiii_ciphertext}')
phaseiii_st_cipher = ''
phaseiii_binary = []
for phaseiii_bytes in range(8):
    phaseiii_binary.append(int(phaseiii_ciphertext[phaseiii_bytes], 2))
    phaseiii_st_cipher += chr(phaseiii_binary[phaseiii_bytes])

print(f'Required Ciphered Text (by Key-1) in ASCII Characters is:{phaseiii_st_cipher}')
print(f'{bcolors.OKCYAN}Left at the end of whole Encryption is:{left}')
print(f'Right at the end of whole Encryption is:{right}')


print(f'{bcolors.BOLD}___________________________DECRYPTION PHASES______________________________')
# Decryption Phase-I
# Previous Result (Cipher Text) x Key 1 | D(K1,C)
# Decryption with K1
decrypt(left,right,_roundkey_arr_K1)

dphasei_plaintext = left + right
print(f'{bcolors.OKGREEN} Phase-I Partial Plain Text (by Key-1) in Bits is: {dphasei_plaintext}')
dphasei_st_plain = ''
dphasei_binary = []
for dphasei_bytes in range(8):
    dphasei_binary.append(int(dphasei_plaintext[dphasei_bytes], 2))
    dphasei_st_plain += chr(dphasei_binary[dphasei_bytes])

print(f'Phase-I Partial Plain Text (by Key-1) in ASCII Characters is:{dphasei_st_plain}')
print(f'{bcolors.OKCYAN}Left at the end of Decryption Phase-I is:{left}')
print(f'Right at the end of Decryption Phase-I is:{right}')

# Decryption Phase-II
# Previous Result x Key 2 | E(K2,D(K1,C))
# Encryption with K2
encrypt(left, right, _roundkey_arr_K2)
dphaseii_plaintext = left + right
print(f'{bcolors.OKGREEN}Phase-II Partial Plain Text (by Key-2) in Bits is: {dphaseii_plaintext}')
dphaseii_st_plain = ''
dphaseii_binary = []
for dphaseii_bytes in range(8):
    dphaseii_binary.append(int(dphaseii_plaintext[dphaseii_bytes], 2))
    dphaseii_st_plain += chr(dphaseii_binary[dphaseii_bytes])

print(f'Phase-II Partial Plain Text (by Key-2) in ASCII Characters is:{dphaseii_st_plain}')
print(f'{bcolors.OKCYAN}Left at the end of Decryption Phase-II is:{left}')
print(f'Right at the end of Decryption Phase-II is:{right}')

# Decryption Phase-III (Final Phase)
# Previous Result x Key 1 | D(K1,E(K2,D(K1,C)))
# Decryption with K1
decrypt(left,right,_roundkey_arr_K1)

dphaseii_plaintext = left + right
print(f'{bcolors.OKGREEN}Required Plain Text (by Key-1) in Bits is: {dphaseii_plaintext}')
dphaseii_st_plain = ''
dphaseii_binary = []
for dphaseii_bytes in range(8):
    dphaseii_binary.append(int(dphaseii_plaintext[dphaseii_bytes], 2))
...
print(f'Required Plain Text (by Key-1) in ASCII Characters is:{dphaseii_st_plain}')
